            ...
            await this._viewService.ExecuteBusyActionAsync(
                async () =>
                {
                    foreach (var c in this.Charges)
                    {
                        ...
                        charge.BilledAmount = c.BilledAmount;
                        charge.ChargeId = c.ChargeId;
                        charge.Description = c.Description;
                        charge.EmployeeId = c.EmployeeId;
                        charge.ExpenseDate = c.ExpenseDate;
                        charge.ExpenseReportId = c.ExpenseReportId;
                        charge.Location = c.Location;
                        charge.Merchant = c.Merchant;
                        charge.Notes = c.Notes;
                        charge.TransactionAmount = c.TransactionAmount;
                        await this._repositoryService.UpdateChargeAsync(charge);

                        Charge newCharge = new Charge();
                        var message = string.Format("Charge was saved on {0:d}, System.DateTime.Today");
                        var message2 = $"Charge was saved on {System.DateTime.Today:d}";
